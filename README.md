...
> 编辑模式默认**关闭**：目标应用可正常无感使用。需要调试时点通知栏通知上的「**开启**」（或长按空白）进入编辑模式；**关闭只通过编辑弹窗里的 Exit Edit Mode（退出编辑模式）按钮**，退出后无任何残留 UI。

1. 在 **LSPosed**（或其他 Xposed 框架）中勾选需要调试的目标应用，勾选后**重启**该应用
2. 打开目标应用，通知栏会出现「编辑模式」常驻通知，显示当前状态（已开启 / 已关闭；需要目标应用有通知权限，Android 13+ 无权限可在系统设置里开启）
3. 点通知上的「**开启**」→ 进入编辑模式；触摸屏幕选中视图 → 修改属性 → **Apply** 应用修改（修改自动持久化，重启后生效）
4. 点击已修改的视图 → **Reset Rule** 可删除规则并恢复原始状态
5. 编辑完成后在弹窗里点 **Exit Edit Mode（退出编辑模式）** → 目标应用恢复正常点击使用，通知同步显示「已关闭」
6. 再次进入编辑模式：点通知上的「**开启**」（已开启时点击无效果）；无通知权限时可在目标应用**长按空白处 ≥1 秒**
...
6. 通知显示"已开启"时点击控件弹编辑弹窗；**Exit Edit Mode** 退出后点击透传、通知变"已关闭"；点通知「开启」重新进入（已开启时点击无效果）
